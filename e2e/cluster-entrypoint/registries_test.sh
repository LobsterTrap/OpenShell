...
setup() {
	...
	unset OPENSHELL_DOCKERIO_MIRROR 2>/dev/null || true
}
...
test_registry_host_and_dockerio_mirror() {
	printf 'TEST: REGISTRY_HOST set + OPENSHELL_DOCKERIO_MIRROR set\n'
	setup

	export REGISTRY_HOST="ghcr.io"
	export OPENSHELL_DOCKERIO_MIRROR="https://mirror.gcr.io"

	OUTPUT=$(run_registry_config)

	assert_file_exists "$REGISTRIES_YAML" "registries.yaml created"
	assert_file_contains "$REGISTRIES_YAML" '"ghcr.io"' "contains ghcr.io mirror"
	assert_file_contains "$REGISTRIES_YAML" '"docker.io"' "contains docker.io mirror"
	assert_file_contains "$REGISTRIES_YAML" 'https://mirror.gcr.io' "contains mirror.gcr.io endpoint"
	assert_output_contains "$OUTPUT" "Adding docker.io mirror" "logs docker.io mirror addition"

	teardown
}

test_registry_host_without_dockerio_mirror() {
	printf 'TEST: REGISTRY_HOST set + OPENSHELL_DOCKERIO_MIRROR unset\n'
	setup
	...
	assert_file_exists "$REGISTRIES_YAML" "registries.yaml created"
	assert_file_contains "$REGISTRIES_YAML" '"ghcr.io"' "contains ghcr.io mirror"
	assert_file_not_contains "$REGISTRIES_YAML" '"docker.io"' "does not contain docker.io mirror"
	assert_output_contains "$OUTPUT" "unqualified image pulls will use docker.io directly" "logs info about direct docker.io pulls"

	teardown
}

test_no_registry_host_with_dockerio_mirror() {
	printf 'TEST: REGISTRY_HOST unset + OPENSHELL_DOCKERIO_MIRROR set\n'
	setup
	...
	assert_file_contains "$REGISTRIES_YAML" '"docker.io"' "contains docker.io mirror"
	assert_file_contains "$REGISTRIES_YAML" 'https://mirror.gcr.io' "contains mirror.gcr.io endpoint"
	assert_file_not_contains "$REGISTRIES_YAML" '"ghcr.io"' "does not contain ghcr.io mirror"
	assert_output_contains "$OUTPUT" "Configuring docker.io mirror" "logs docker.io mirror configuration"
	assert_output_contains "$OUTPUT" "REGISTRY_HOST not set" "logs warning about missing REGISTRY_HOST"

	teardown
}

test_no_registry_host_no_dockerio_mirror() {
	printf 'TEST: REGISTRY_HOST unset + OPENSHELL_DOCKERIO_MIRROR unset\n'
	setup
...
test_dockerio_mirror_with_auth() {
	printf 'TEST: REGISTRY_HOST + OPENSHELL_DOCKERIO_MIRROR + auth credentials\n'
	setup
...
test_dockerio_mirror_valid_yaml_structure() {
...
test_registry_host_and_dockerio_mirror;        echo ""
test_registry_host_without_dockerio_mirror;    echo ""
test_no_registry_host_with_dockerio_mirror;    echo ""
test_no_registry_host_no_dockerio_mirror;      echo ""
test_dockerio_mirror_with_auth;                echo ""
test_dockerio_mirror_valid_yaml_structure
